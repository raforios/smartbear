...
def _monthly_frame(months: int, amount: float = 100.0) -> pd.DataFrame:
    ...
    '''
    return pd.DataFrame({
        'fecha': pd.date_range('2023-01-01', periods = months, freq = 'MS'),
        'id_pedido': [f'F{index}' for index in range(months)],
        'monto_total': [amount] * months,
        'categoria': ['CAFES'] * months,
    })
...
def test_month_over_month_variation_is_reported():
    ...
    frame.loc[2, 'monto_total'] = 200.0
    kpis = {kpi['label']: kpi['value'] for kpi in build_growth(frame)['kpis']}
    assert kpis['Variación vs mes anterior'] == 100.0
...
def test_seasonality_needs_a_full_year():
    ...
    '''
    assert not build_growth(_monthly_frame(6))['estacionalidad']
    assert len(build_growth(_monthly_frame(12))['estacionalidad']) == 12


def test_seasonality_index_centers_on_one_hundred():
    '''With identical months every index is exactly the average, i.e. 100.'''
    indices = [row['indice'] for row in build_growth(_monthly_frame(12))['estacionalidad']]
...
def test_monthly_series_has_no_variation_on_its_first_point():
    ...
    '''
    series = build_growth(_monthly_frame(4))['variacion_mensual']
    assert series[0]['variacion'] is None
    assert series[1]['variacion'] == 0.0


def test_empty_sections_when_dates_are_missing():
    '''A file without a usable date column yields empty sections, not an error.'''
    frame = _monthly_frame(3).drop(columns = ['fecha'])
    result = build_growth(frame)
    assert not result['variacion_mensual']
    assert not result['estacionalidad']
