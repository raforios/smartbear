...
def get_all_planned_routes(
    ...
    try:
        ...
        error_msg = f'Failed to get all planned routes: {e}'
        logger.error(error_msg, exc_info = True)
        raise
...
def delete_planned_point(
    ...
    try:
        ...
        db.rollback()
        error_msg = f'Failed to delete point {planned_point_id} from route {planned_route_id}: {e}'
        logger.error(error_msg, exc_info = True)
        raise
...
def get_statistics_user_points(
    ...
    try:
       ...
       # Compile a list of details for all visited points
        ...
        error_msg = f'Failed to get user point statistics for user {user_id}: {e}'
        logger.error(error_msg, exc_info = True)
        raise
...
def get_route_comparisons(
    ...
    try:
        ...
        error_msg = f'''Failed to get route comparisons for planned route
                {planned_route_id}: {e}'''
        logger.error(error_msg, exc_info = True)
        raise
...
def get_full_route_comparison(
    ...
    try:
        ...
        error_msg = f'Failed to get full route comparison for planned route {planned_route_id}: {e}'
        logger.error(error_msg, exc_info = True)
        raise
