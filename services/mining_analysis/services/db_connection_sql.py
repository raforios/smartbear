'''
    ...
    The service publishes the quotations from DynamoDB — that is what
    `db_connection.py` serves, the same boilerplate as every other service.
    This file is the second engine: the Ministry's royalties tables and the
...
# refuses a missing or empty required variable, so the loop that used to be
