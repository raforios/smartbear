'''
    Mining Summit Microservice Main Handler
'''
import socket
...
async def lifespan(_: FastAPI) -> AsyncIterator[None]:
    ...
    '''
    message = 'Mining Summit startup: DynamoDB-backed service ready.'
    logger.info(message)
    yield
    message = 'Mining Summit shutdown: closing resources.'
    logger.info(message)


APP_CONFIG = {
    'root_path': ROOT_PATH_NORMALIZED,
    'title': 'Mining Summit Service',
    'description': (
        'Microservice that powers the "Cumbre Minera" event: participant '
        'registration, daily attendance check-ins, listing reports filtered by '
        'CI and date range, and statistical aggregations by department / company.'
    ),
    'version': '1.0.0',
    'contact': {
        'name': 'API Support',
