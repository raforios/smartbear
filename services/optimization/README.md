...
y proyecta el resultado sobre la red vial de OpenStreetMap usando `osmnx` y
`networkx`. **Reemplaza** los endpoints `/api/v1/optimization/*` del monolito
legado (`api/routes/optimization.py`), preservando el contrato.
...
- AWS DynamoDB (`boto3`)
- `osmnx` + `networkx` para análisis del grafo vial
...
├── Dockerfile          # Build para Lambda (osmnx + geos + proj).
...
| `GET` | `/v1/optimization/optimal_route?route_id&day&dist` | Ruta optimizada proyectada sobre la red vial de OSM. |
...
2. El cálculo de distancias y la proyección a la red vial son **idénticos** al
   monolito legado — la migración no cambia los resultados.
3. `osmnx.graph_from_point` golpea servidores públicos de OSM en runtime; en
   producción evaluar pre-computar grafos y guardarlos en S3 para evitar
   rate-limits y reducir latencia.
