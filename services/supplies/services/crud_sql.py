'''
    Generic CRUD for the relational (MySQL) side of the service: the warehouse
    catalogue, suppliers, entries and kardex. The DynamoDB CRUD the pharmacy
    product uses is `crud.py`, the shared boilerplate.
'''
...
def create_record(
    ...
    try:
        ...
        if any(isinstance(v, list) and
            ...
            error_msg = f'Data dict for {model.__name__
...
def get_record(
    ...
    try:
        ...
        if not record:
            error_msg = f'{model.__name__} with ID {record_id} not found.'
            logger.warning(error_msg)
