'''
    ...
    The pharmacy product runs on DynamoDB — that is what `db_connection.py`
    serves, the same boilerplate as every other service. This file is the
    second engine: the warehouse catalogue, entries and kardex delivered to
    the Ministry still live on MySQL, so their routes ask for
...
# No second guard over DB_PARAMETERS: load_and_validate_env_vars already
# refuses a missing or empty required variable, so the loop that used to
# be here could never fire.
...
GET_SQL_DB_DEPENDENCY: Callable = get_db_session(ENGINE)  # pylint: disable=invalid-name
